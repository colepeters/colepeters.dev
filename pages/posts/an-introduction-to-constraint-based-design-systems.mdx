...
import cube from '../../public/images/design-systems/cube.jpg'
import spacingScale from '../../public/images/design-systems/spacing-scale.jpg'

<PostWrapper title='An Introduction to Constraint Based Design Systems' description='Effective designs optimize for constraints. These constraints are not just useful, they are critical: a beautiful building not designed for the constraints of tectonic movement and climate variability will collapse, and products not designed for the myriad constraints of the web will never amass a critical user base. Furthermore, a design system without adequate constraints will quickly grow into a disordered mess, especially as more contributors join in and more design problems are encountered.'>

# An Introduction to Constraint Based Design&nbsp;Systems

Over the course of my 15 years of work in design and engineering, I’ve spent an enormous amount of time thinking about, talking about, building, and maintaining design systems — the structures that make up the foundation of modern user interfaces on the web. Particularly as I’ve taken on work at [the staff-plus level of engineering](https://staffeng.com/guides/what-do-staff-engineers-actually-do), I’ve made it my responsibility to make sure my team(s) are all working from a shared technical perspective. This is especially important for designers and frontend engineers, as they operate in distinct but overlapping domains.
...
These challenges, among others, are what make the successful implementation of design systems a necessity for teams collaborating on digital products. When designers and engineers operate from a shared point of departure, they unlock the ability to create work that is inherently consistent, repeatable, flexible, and greater than either could devise in isolation.

## Another design systems&nbsp;article?

Over the past decade, the topic of design systems has exploded — once confined to niche conversations on isolated corners of the internet, there are now multiple high profile conferences focusing exclusively on design systems for digital products. As such, you’ll have no trouble finding a litany of articles and presentations on this subject. Does the internet really need another design systems article?

This article is one born from years of personal experience. I first set these thoughts down in 2019 while working for a startup on the edge of substantial growth. Up to that point, I had served as one of only two frontend engineers at the company, but with multiple design and engineering hires on the horizon, I knew that onboarding them to the design system I had built (in collaboration with our creative director) would be critical in helping them to hit the ground running. Many of the hires we made, on both the engineering and design side, had never worked with any form of design system before, and the remainder had never worked with a system quite like ours, particularly with respect to its focus on constraints.

With all of this in mind, I decided to create an introduction to design systems from the ground up — one that would be digestible for both designers and engineers. A great many articles on design systems are directed at _either_ designers _or_ engineers, and as such, they often make use of terminology and domain knowledge that may not be familiar to the ‘visiting party’. I wanted to be able to get _both_ our designers _and_ our frontend engineers in the same room (or video chat), learning from the same material, and on the same page from day one. I did this not only to make the most of my time spent onboarding new staff, but more importantly, to set the stage for tight collaboration from the outset of our new hires’ experiences. The team that learns together also grows together.

Accordingly, this article goes heavy on theory and lighter on application. While some implementation details are necessarily touched on, this article is not meant to inform the specific implementation of a design system. Rather, my intent is to present a rigorous conceptual foundation from which any effective design system on the web may be constructed or analyzed.

The approach behind this article is one that I’ve used with great success over the past 3 years while onboarding and mentoring multiple designers and engineers. The individuals and teams I’ve worked with have repeatedly expressed both a genuine enjoyment of the material and a deepened sense of collaboration and understanding with their cross-functional colleagues as a result. My hope is that by making this article publicly available (and updating it substantially in the process), many more designers and engineers will find it to be a useful reference in their own work and relationships.

At its core, this article draws deeply on my own work in crafting successful design systems at multiple organizations over the past 8 years, as well as my experiences as a designer/engineer hybrid in the years before that. My work, in turn, has particularly benefitted from learning from [Adam Morse](https://mrmrs.cc/) and [Brent Jackson](https://jxnblk.com/), and I would be remiss not to thank them for their inspiration, guidance, and incredible setting of examples through the years.
...
The above description of design systems provides a good theoretical starting point, but how do design systems manifest in practice? This answer will vary between different fields of study (city planning, architecture, graphic design, computer programming, etc.), but when it comes to digital products, I define as design system as containing:

1. **A brand design specification** (high-level typography, colour palette, layout sensibilities, etc.)
2. **A reusable component library** — that is, a collection of components which convey that brand design through practical applications (discrete typographic elements, iconography, interactive elements, etc.)
3. **A modular codebase**, which turns these components into reality for end users
...
How many times have you been working on a layout (either as a designer or an engineer) and thought to yourself, ‘How many pixels should _this_ be placed away from _that_?’ How many times have you gone with your gut, trying out variations of numbers until it ‘looked right’? How many times have you had to ask your designer for a redline, or ask your engineer to change a 13 pixel margin to a 14 pixel margin?
...
Methods for implementing these kinds of constraints within an organization’s design tools and codebase can be nearly as varied as the composition of the design systems themselves. However, given the explosion of interest in design systems in recent years, a growing number of options are gaining popular support. [Design token plugins for Figma](https://www.figma.com/community/tag/design%20tokens/plugins), for example, are providing designers with programmatic ways of codifying and using design system constraints within their wireframes, mockups, and prototypes. As a frontend engineer, I’ve been making use of (and loving) Brent Jackson’s [Styled System library](https://styled-system.com/) since it first emerged in 2017; [Theme UI](https://theme-ui.com/) is another appealing option. For those not working in React, a constraint based design system can easily be implemented in vanilla CSS — [Tailwind](https://tailwindcss.com/), a popular functional CSS library, is even [geared towards use within design systems](https://tailwindcss.com/docs/configuration).
...
>
  ...
  <Box role='rowgroup'>
    <Box
      ...
      p={[2, 3]}
    >
      <Text as='ol' role='cell' my={0} pl={0} fontSize={1}>
